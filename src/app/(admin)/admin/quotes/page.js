...
const Expending = () => {
  ...
  const columns = [
    ...
    },
    {
      name: 'Action',
      cell: row => <>
        <Link href={`/admin/quotes/edit/${row.id}`}><Button className="w-10 h-6 text-xs bg-opacity-80 bg-black">Edit</Button></Link>
        <DropdownMenu>
  ...
  const handleDelete = () => {
    ...
    try {
      axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/quote/${selectedRow.id}`, {
        withCredentials: true,
      })
        .then(response => {
      ...
      <div className="flex min-h-screen pt-10">
        ...
        <AlertDialogContent>
          ...
          <div className="p-4">
            ...
            <div className='mb-2'>
              <Link href="/admin/quotes/add/0"><Button className="bg-black">Add</Button></Link>
