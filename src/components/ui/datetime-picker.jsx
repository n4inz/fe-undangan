...
/**
 * Convert a 12-hour input to 24-hour format.
 * 12:00 PM is 12:00 and 12:00 AM is 00:00.
 */
...
/**
 * Display the stored 24-hour time in 12-hour representation.
 */
...
function Calendar({
  ...
  return (
    <DayPicker
      // defaultMonth={new Date(1999, 0, 1)} // calendar starts in January 1999
        ...
        MonthCaption: ({ calendarMonth }) => {
          return (
            <div className="inline-flex gap-2">
              <Select
                // defaultValue={props.selected ? getMonth(props.selected).toString() : calendarMonth.date.getMonth().toString()}
  ...
  (
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key >= "0" && e.key <= "9") {
        ...
        if (flag) onRightFocus && onRightFocus();
        setFlag((prev) => !prev);
        const tempDate = date ? new Date(date) : new Date();
        onDateChange &&
          onDateChange(setDateByType(tempDate, newValue, picker, period));
    ...
    return (
      <div className="flex items-center justify-center gap-2">
        ...
        <TimePickerInput
          ...
          ref={hourRef}
          period={period}
          onRightFocus={() => minuteRef.current && minuteRef.current.focus()}
        />
        {(granularity === "minute" || granularity === "second") && (
          <>
            ...
            <TimePickerInput
              ...
              onDateChange={onChange}
              ref={minuteRef}
              onLeftFocus={() => hourRef.current && hourRef.current.focus()}
              onRightFocus={() =>
        ...
        {granularity === "second" && (
          <>
            ...
            <TimePickerInput
              ...
              onDateChange={onChange}
              ref={secondRef}
              onLeftFocus={() => minuteRef.current && minuteRef.current.focus()}
              onRightFocus={() =>
                periodRef.current && periodRef.current.focus()
              }
            />
          </>
        )}
        {hourCycle === 12 && (
          <div className="grid gap-1 text-center">
            <TimePeriodSelect
              ...
              ref={periodRef}
              onLeftFocus={() => secondRef.current && secondRef.current.focus()}
            />
          </div>
        )}
      </div>
  ...
  (
    ...
    const handleMonthChange = (newDay) => {
      ...
      if (!defaultPopupValue) {
        ...
        onMonthChange && onMonthChange(newDay);
        setMonth(newDay);
        return;
      }
      ...
      onMonthChange && onMonthChange(newDateFull);
      setMonth(newDateFull);
    };

    const onSelect = (newDay) => {
      if (!newDay) return;
      onChange && onChange(newDay);
      setMonth(newDay);
...
useEffect(() => {
  console.log(month);
    ...
    const initHourFormat = {
      ...
      hour12:
        (displayFormat && displayFormat.hour12) ||
