    ...
    if (id && code && code !== null && code !== undefined) {
      ...
      try {
        const response = await axios.delete(
          `${process.env.NEXT_PUBLIC_API_URL}/remove-image-ak/${id}/${params.formId}`
        );
  ...
  const manageOrder = async (images) => {
    ...
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/manage-bg-order-ak/${params.formId}`,
        images
      );
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/get-gallery-ak/${params.formId}`,
      ...
      const imagesData = response.data.data
        .map((item) => {
          const filename = item.images?.fileImage || item.asset?.file;

          if (!filename) return null;

          return {
            url: `${process.env.NEXT_PUBLIC_API_URL}/images/${filename}`,
            id: item.id,
            file: null,
